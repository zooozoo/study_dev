...
- 커밋·푸시는 스킬이 마무리 단계에서 수행한다 — 이번 노트 작업 파일만 지정해 스테이징(`git add -A` 금지), 커밋 메시지는 `docs: <주제> 학습 노트 추가|보강`, 작성자는 repo 로컬 git 설정(개인 계정) 그대로, `git push origin <현재 브랜치>`. 사용자가 "커밋하지 마"라고 한 경우만 메시지 제안으로 그친다. 커밋 전에 위 내용 경계를 한 번 더 확인한다.
