...
> 작성일: 2026-09-10 · 갱신일: 2026-09-11 · AWS 공식 문서 확인일: 2026-09-10
...
## Dev Weekly 한 장 요약 — 계정과 로그인 주체를 두 축으로 보기

AWS 구조가 헷갈리는 가장 큰 이유는 **리소스를 담는 계정**과 **그 계정에 로그인하는 사람·권한**을 같은 것으로 생각하기 때문이다. 먼저 아래 두 축을 분리하면 나머지 용어의 자리가 보인다.

| 구분 | 답하려는 질문 | 핵심 개념 |
|---|---|---|
| **계정·조직 축** | 리소스는 어디에 있고, 조직에서 이 계정의 위치는 어디인가? | AWS 계정, Organizations, 관리 계정, 멤버 계정, OU, SCP |
| **로그인·권한 축** | 누가 어떤 방법과 권한으로 그 계정에 들어가는가? | 루트 사용자, IAM 사용자, IAM 역할, Identity Center 사용자 |

**AWS 계정**은 EC2·RDS·S3 같은 리소스와 비용이 담기는 독립된 그릇이다. 계정 자체는 사람이 아니며, 사람이 계정 안에서 작업하려면 로그인 주체로 인증되어야 한다. 여러 계정은 **AWS Organizations**로 묶는다. 조직을 만든 **관리 계정**은 조직·결제·공통 정책을 관리하고, 나머지 **멤버 계정**은 운영·개발·보안처럼 실제 워크로드를 분리해 담는다. 따라서 관리 계정과 멤버 계정은 **조직 안에서 계정의 위치를 구분하는 말**이다.

반면 루트 사용자와 관리자는 로그인·권한 축의 개념이다. **루트 사용자**는 AWS 계정의 소유자 신원이고, **관리자**는 별도의 사용자 종류가 아니라 사용자나 역할에 넓은 권한을 부여한 상태다. 따라서 관리 계정에도 일반 사용자나 역할이 있을 수 있고, 멤버 계정에도 루트 사용자 신원이 있다. 멤버 계정의 루트 사용자가 자기 계정에서 강한 권한을 가진다고 해서 조직 전체를 관리할 수 있는 것은 아니다.

로그인 주체 네 가지는 다음처럼 구분한다.

| 로그인 주체 | 어디에 존재하나 | 주된 용도 | 기억할 점 |
|---|---|---|---|
| **루트 사용자** | AWS 계정 하나 | 계정 소유자만 가능한 비상 작업 | 일상 작업에 사용하지 않는다 |
| **IAM 사용자** | 특정 AWS 계정 안 | 한 계정에 직접 로그인하는 장기 신원 | 계정이 늘면 사용자와 자격증명도 따로 관리해야 한다 |
| **IAM 역할** | 특정 AWS 계정 안 | 사람·서비스·다른 계정이 임시로 권한을 맡아 작업 | 비밀번호 없이 임시 자격증명을 사용한다 |
| **Identity Center 사용자** | 조직의 중앙 로그인 체계 | 직원이 한 번 로그인해 여러 계정에 접근 | 대상 계정에서는 Identity Center가 만든 IAM 역할을 맡는다 |

실무에서 권장되는 흐름은 **직원 → IAM Identity Center → 대상 계정의 IAM 역할 → AWS 리소스**다. 사용자는 중앙에서 한 번 로그인하고, 자신에게 배정된 계정과 권한 세트(Permission Set)를 고른다. 그러면 대상 계정의 역할을 맡은 임시 세션으로 들어간다. 한편 **SCP(Service Control Policy)** 는 권한을 새로 주는 정책이 아니라, 조직·OU·계정에서 사용할 수 있는 권한의 최대 범위를 제한하는 안전장치다.

![AWS 계정과 로그인 주체를 두 축으로 구분한 구조도](aws-account-and-login-identities-overview.png)

한 문장으로 요약하면, **계정은 리소스를 분리하는 그릇이고, 사용자와 역할은 그 그릇에 들어가는 방법이며, Organizations와 Identity Center는 각각 계정과 사람을 중앙에서 관리하는 체계다.**
